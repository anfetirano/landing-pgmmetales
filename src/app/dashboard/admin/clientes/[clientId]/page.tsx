...
export default function AdminClientSheetPage() {
  const { user } = useUser();
  ...
  const sheet = useQuery(
    api.purchases.getClientSheetByAdmin,
    dbUser?._id && clientId ? { adminId: dbUser._id, clientId } : "skip"
  );
    ...
    <div className="max-w-5xl grid gap-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 text-sm">
          <div>Comprador: {sheet.client.buyerName ?? "-"}</div>
